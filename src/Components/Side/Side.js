...
export const Side = () => {
  ...
  const {  pp, setPp } = useContext(UserContext);

  useEffect(() => {
    const fetchProfilePic = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/check/${name}`);
        ...
        setFinalProfilePic(profilepic1); // Fallback image
      }
    };

    if (!profilePic) fetchProfilePic();
  }, [name, profilePic]);
    ...
    <div className='Side'>
      <ul className='sideul'>
        ...
        <li className='NotHome sideli'>
          <Link to={`/${username||name}`} className='btn'>
        ...
        <li className='NotSearch sideli'>
          <Link to={`/${username||name}/search`} className='btn'>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" viewBox="0 0 32 32">
        ...
        <li className='NotNotifications sideli'>
          <Link to={`/${username||name}/notifications`} className='btn'>
        ...
        <li className='NotBookmark sideli'>
          <Link to={`/${username||name}/bookmarks`} className='btn'>
          <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="32" height="32" viewBox="0 0 32 32">
